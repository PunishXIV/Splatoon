...
{
    private delegate nint EventFramework_ProcessSystemLogMessageDelegate(void* eventFramework, uint a2, uint a3, nint a4, byte a5);
    [EzHook("40 55 56 41 54 41 55 41 57 48 8D 6C 24 ?? 48 81 EC ?? ?? ?? ?? 48 8B 05 ?? ?? ?? ?? 48 33 C4 48 89 45 FF")]
    private EzHook<EventFramework_ProcessSystemLogMessageDelegate> EventFramework_ProcessSystemLogMessageHook;
    ...
    private nint EventFramework_ProcessSystemLogMessageDetour(void* eventFramework, uint a2, uint messageId, nint a4, byte a5)
    {
        try
        {
