...
internal class P2_Missing_1234_5678 : SplatoonScript
{
    #region Metadata
    ...
    private void DrawPrioritySettings()
    {
        ...
        ImGui.TextUnformatted(
            $"Pattern detection uses debuffs on the active half only (steps {FirstHalfStepMin}-{FirstHalfStepMax}: first, {SecondHalfStepMin}-{SecondHalfStepMax}: second).");
    ...
    {
        if(_step is >= FirstHalfStepMin and <= FirstHalfStepMax)
        {
            half = MechanicHalf.First;
            return true;
        }

        if(_step is >= SecondHalfStepMin and <= SecondHalfStepMax)
        {
