...
{
    public override Metadata Metadata { get; } = new(6, "NightmareXIV, mirage");
    ...
    public override void OnUpdate()
    {
        ...
        bool spread = false;
        {
            ...
            {
                hints.Add((Str_Spread($"{time.SafeSelect(0).Time:F1}"), time.SafeSelect(0).Time));
                spread = true;
            }
        }
        {
            ...
            {
                {
                    ...
                    {
                        hints.Add((Str_Stack($"{time.SafeSelect(0).Time:F1}"), time.SafeSelect(0).Time));
                        break;
                    }
                }
                {
        ...
        }
        {
            ...
            {
                hints.Add((!this.FakeStatuses.ContainsAny(Debuffs.DebuffDontMove.Select(s => new StatusInfo(BasePlayer.ObjectId, s))) ? Str_DontMove($"{time.SafeSelect(0).Time:F1}") : Str_Move($"{time.SafeSelect(0).Time:F1}"), time.SafeSelect(0).Time));
            }
        }
        {
    ...
    {
        var e = Controller.GetRegisteredLayouts()["Language"].GetElement(element)!;
