...
public class M11S_Majestic_Meteor_Body_Language_Edition : SplatoonScript
{
    ...
    private readonly HashSet<uint> _castArcadianCrashResolve = new() { 46155, 46157, 46159, 46161 };
    ...
    private readonly Vector2 _safeNWBase = new(80f, 85f);
    ...
    private IEnumerable<uint> _upOrDown  = new List<uint>();
    private bool _isUp = false;
    private uint _castId = 0;
    ...
    public override void OnStartingCast(uint source, uint castId)
    {
        ...
        else if (_state == State.ArcadionAvalance && _castArcadianCrash.Contains(castId))
        {
    ...
    public override void OnUpdate()
    {
        ...
        if (_state is  State.MarkerBait or State.Puddles1 or State.Puddles2 or State.Puddles3)
        {
            ...
            if (_state == State.MarkerBait)
            {
                ...
                stepOffset = _isUp ? -stepOffset+6 : +stepOffset-6;

                var pos = new Vector2(posX, 100f + stepOffset);
                var text = _isUp ? "UpNext" : "DownNext";
            ...
            if (_state is State.Puddles1 or State.Puddles2 or State.Puddles3)
            {
                ...
                stepOffset = _isUp ? -stepOffset : +stepOffset;
                stepOffsetNext = _isUp ? -stepOffset-6 : +stepOffset+6;

                var pos = new Vector2(posX, 100f + stepOffset);
                var posNext = new Vector2(posX, 100f + stepOffsetNext);
                var text = _isUp ? "Up" : "Down";
                var textNext = _isUp ? "UpNext" : "DownNext";
        ...
        if (_state == State.ArcadionAvalance)
        {
            ...
            var isNorthTower = dir is Direction.NorthEast or Direction.NorthWest;
            Vector2 stand;
            if (isEast && _notSafe is Direction.SouthEast or Direction.NorthEast)
            {
                same = true;
            }
            else if (isEast && _notSafe is Direction.NorthWest or Direction.SouthWest)
            {
                same = false;
            }
            else if (!isEast && _notSafe is Direction.SouthEast or Direction.NorthEast)
            {
                same = false;
            }
            else if (!isEast && _notSafe is Direction.NorthWest or Direction.SouthWest)
        ...
        if (_state == State.ArcadionAvalanceGoSafe)
        {
            var safe = _notSafe switch
            {
                Direction.SouthEast => SafeNE(),
                Direction.NorthEast => SafeSE(),
                Direction.SouthWest => SafeNW(),
                Direction.NorthWest => SafeSW()
            };
    ...
    private Vector2 SafeNW()
    {
        return _safeNWBase;
    }

    private Vector2 SafeNE()
    {
        return MirrorX(_safeNWBase);
    }

    private Vector2 SafeSW()
    {
        return MirrorZ(_safeNWBase);
    }

    private Vector2 SafeSE()
    {
        return MirrorXZ(_safeNWBase);
    }
    ...
    public override void OnSettingsDraw()
    {
        ...
        ImGuiEx.HelpMarker("""
            ...
            The first TowerBait is based on player realtime positions
        """);
