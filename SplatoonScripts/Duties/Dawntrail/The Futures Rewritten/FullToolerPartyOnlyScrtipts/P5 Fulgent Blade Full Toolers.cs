...
internal class P5_Fulgent_Blade_Full_Toolers :SplatoonScript
{
    ...
    public override void OnSetup()
    {
        ...
        Controller.RegisterElement($"Line2", new Element(2) { radius = 0f, thicc = 6f, fillIntensity = 0.5f });
    }
    ...
    public override void OnStartingCast(uint source, uint castId)
    {
        if (castId == 40316)
        {
            ...
            float range = (pc.Index == _firstLightPcIndex || pc.EntityId == _firstDarkPcIndex) ? 4f : 8f;
            if (bait1 == "L") ApplyElement("Bait", _clockDirectionCalculator.GetDirectionFromClock(7), range);
            if (bait1 == "R") ApplyElement("Bait", _clockDirectionCalculator.GetDirectionFromClock(4), range);
            _state = State.Stack1;
        }
