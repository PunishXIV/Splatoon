...
public unsafe class P5_Fulgent_Blade_Dodge : SplatoonScript
{
    ...
    private long MechanicStartTime = 0;
    private long Phase => Environment.TickCount64 - MechanicStartTime;

    private static Vector2[] DodgeCw = [new(5.700f, 10.800f), new(0.800f, 11.300f), new(2.3f, 6.2f), new(6.4f, 9.6f)];
    private static Vector2[] DodgeCcw = [new(-5.500f, 12.900f), new(-0.400f, 12.300f), new(-2.400f, 6.100f), new(-5.600f, 10.700f)];

    private Vector2[] DodgesCcw = [
        DodgeCcw[0],
        DodgeCcw[1],
        DodgeCcw[1],
        DodgeCcw[2],
    ...
    private IBattleNpc[] Npcs => Svc.Objects.OfType<IBattleNpc>().Where(x => x.NameId.EqualsAny<uint>(13561) && x.Struct()->GetCastInfo() != null && x.CastActionId.EqualsAny<uint>(40118, 40307) && x.IsCasting).ToArray();

    private (Vector3 Position, float Rotation)? Reference = null;
    ...
    public override void OnUpdate()
    {
        ...
        if(Reference != null)
        {
            ...
            d.overlayText = "Ref";
            for(var i = 0; i < DodgesCcw.Length; i++)
            {
