...
namespace SplatoonScriptsOfficial.Duties.Endwalker
{
    public unsafe class Aloalo_Bombs : SplatoonScript
    {
        ...
        {
            if(source.GetObject()?.DataId == BombData)
            {
        ...
        {
            if(set.Target?.DataId == BombData && set.Action?.RowId == 35165)
            {
                Reset();
            }
        }
        ...
        nint ActionTimelineManager_unkDetour(nint a1, uint a2, int a3, nint a4)
        {
            try
            {
                ...
                if(x->Parent != null && x->Parent->GameObject.DataID == BombData && a3 == 1)
                {
