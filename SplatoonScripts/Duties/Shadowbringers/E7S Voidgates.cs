...
public class E7S_Voidgates : SplatoonScript
{
    ...
    bool IsDarkRed = false;

    bool IsUnjoined => Controller.GetPartyMembers().Any(x => x.StatusList.Any(s => s.StatusId.EqualsAny<uint>(2239u, 2238u)));

    public override void OnActionEffectEvent(ActionEffectSet set)
    {
        if(set.Source is IBattleNpc npc)
        {
            //PluginLog.Information($"Action :{ExcelActionHelper.GetActionName(set.Action?.RowId ?? 0, true)}");
            if(IsUnjoined)
            {
                //it's dark and light
                if(set.Action?.RowId.EqualsAny(19554u, 19546u, 19555u) == true) //light
                {
                    float distance = 999f;
                    foreach(var x in ActivePortalDestinations)
                    {
                        ...
                        if(dist < distance && dist < 0.5f) //light went into blue
                        {
                            distance = dist;
                            IsDarkRed = x.Value;
                        }
    ...
    public override void OnUpdate()
    {
        ...
        if(IsUnjoined)
        {
            foreach(var x in ActivePortalSources)
            {
                ...
                var isMeBlue = BasePlayer.StatusList.Any(x => x.StatusId == 2239u);
                var yes = IsDarkRed;
                if(isMeBlue) yes = !yes;
                if(Controller.TryGetElementByName(GetLineByCoord(yes ? redPortal : bluePortal), out var e))
                {
                    e.Enabled = true;
                }
            }
